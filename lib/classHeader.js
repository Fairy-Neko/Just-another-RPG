...
// OMG we need to write a collision system from scratch ......
// Babylon.js cannot create sperate collision boxes & it is difficult to create them for sprites ......
// And it also cannot detect collision between known mesh and unknown meshes, in the API
// e.g. OnColliderEnter() in Unity etc.
// Babylon.js can only do CheckCollision(mesh1, mesh2) and return you a value ...

// The sprite can neither rotate......OMG
// What should we do......

// TODO List:
//  < > Base collider class for collision detection
//  < > OBB and Sphere (circle) colliders
//  < > Interfaces (maybe callback) like OnColliderEnter(collider) etc.
//  ____Some structure to manage all of the colliders in current scene,
//  ____e.g. collider tags, quadtrees, etc:
//  < > Collider tags and grouping
//  < > Quadtree (OMG)
...
class CollisionMgr
{
    ...
    static Singleton()
    {
        ...
        {
            CollisionMgr.singleton = new CollisionMgr();
        }

        return CollisionMgr.singleton;
    }
    ...
    addObject(collider, isPassive = true, {
        tag = "default"
    } = {})
    {
